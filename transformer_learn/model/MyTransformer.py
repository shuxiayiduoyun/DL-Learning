...
class MyTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        :return
        """
        src2 = self.self_attn(src, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask, )[0]  # 计算多头注意力
...
def multi_head_attention_forward(query,  # [tgt_len,batch_size, embed_dim]
    ...
    if is_print_shape:
        print("" + "=" * 80)
        print("进入多头注意力计算:")
        print(f"\t 多头num_heads = {num_heads}, d_model={query.size(-1)}, d_k = d_v = d_model/num_heads={query.size(-1) // num_heads}")
