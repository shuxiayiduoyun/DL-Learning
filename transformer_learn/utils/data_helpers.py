...
def build_vocab(tokenizer, filepath, min_freq=1, specials=None):
    ...
    # 3. 使用小写的 vocab 工厂函数
    custom_vocab = vocab(ordered_dict, min_freq=min_freq, specials=specials)

    # 4. 明确将词表外 token 的默认索引设置为 '<unk>' 的索引
    custom_vocab.set_default_index(custom_vocab['<unk>'])
...
class LoadEnglishGermanDataset():
    def __init__(self, train_file_paths=None, tokenizer=None, batch_size=2, min_freq=1):
        ...
        self.PAD_IDX = self.de_vocab['<pad>']  # 1
        self.BOS_IDX = self.de_vocab['<bos>']  # 2
        self.EOS_IDX = self.de_vocab['<eos>']  # 3
    ...
    def load_train_val_test_data(self, train_file_paths, val_file_paths, test_file_paths):
        ...
        valid_iter = DataLoader(val_data, batch_size=self.batch_size, shuffle=False, collate_fn=self.generate_batch)
